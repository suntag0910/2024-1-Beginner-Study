...
- 답: 1번은 병합되는 브랜치의 커밋 히스토리를 유지하며 병합하는 반면, 2번은 여러개의 커밋을 하나의 커밋으로 합치기 때문에 히스토리가 간소화됨. 3번은 브랜치가 분기된 지점을 새로 지정한다는 차이가 있다.
...
- 답: commit ID a1s2d3f 라는 사실, 현재 작업중인 브랜치, 가장 최근 commit
...
- 답: revert를 선택할 것이다. reset을 사용했다가 자칫 잘못하여 커밋 기록이 완전히 사라져서 다른 사람의 커밋과 충돌이 생길 수도 있기 때문에 돌리기만 하는 revert를 사용할 것이다.